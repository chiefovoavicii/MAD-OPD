#!/bin/bash
# MAD-OPD: K=2 teachers debate for R=2 rounds, confidence-weighted JSD.
# Paper defaults (Qwen3-14B + Qwen3-8B → Qwen3-4B); override via env or
# pass --flag value at the end to forward to ``mad_opd.cli.train``.
#
...
# ``max_length`` caps the prompt; ``max_completion_length`` caps the student
# generation.  The 32k teacher vLLM / 40960 sidecar budgets above cover both.
set -e
cd "$(dirname "$0")/.."
...
OUTPUT=${OUTPUT:-./outputs/mad_opd}

# The paper-default agent run uses the registered ToolACE preprocessor so the
# raw trajectories are split into per-assistant-turn training examples. An
# explicitly supplied DATASET remains untouched for code or custom datasets.
if [[ -z ${DATASET+x} ]]; then
...
torchrun --nproc_per_node=$(get train_nproc_per_node) --master_port=29500 \
    ...
    --seq_kd false --lmbda 1 --beta 0.5 \
    --temperature 0.7 --top_p 0.8 --top_k 20 --repetition_penalty 1.2 --enable_thinking false \
    --mad_debate_rounds 2 --mad_dynamic_weights true --teacher_weights 0.5 0.5 \
    --mad_debate_temperature 0.7 --mad_debate_top_p 0.8 --mad_debate_top_k 20 \
