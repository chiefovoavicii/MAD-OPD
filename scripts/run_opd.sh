#!/bin/bash
# OPD: single-teacher on-policy distillation baseline.
#
...
T1_URL=$(get teacher_1_url) S1_URL=$(get sidecar_1_url)
...
torchrun --nproc_per_node=$(get train_nproc_per_node) --master_port=29500 \
    -m mad_opd.cli.train \
    --rlhf_type gkd --gkd_algorithm opd \
    ...
    --vllm_teacher_1_url "$T1_URL" --vllm_sidecar_1_url "$S1_URL" \
